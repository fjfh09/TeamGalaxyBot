...
export default {
    ...
    async execute(client, oldState, newState) {
        ...
        const CREAR_TRIO_CHANNEL_ID = '863446893149290506'; // Reemplaza esto con el ID del canal "crear trio"

        // Verifica si el usuario se ha unido al canal "crear trio"
        if (newState.channelId === CREAR_TRIO_CHANNEL_ID && oldState.channelId !== CREAR_TRIO_CHANNEL_ID) {
            try {
                ...
                const crearTrioChannel = guild.channels.cache.get(CREAR_TRIO_CHANNEL_ID);

                // Crea un nuevo canal de voz con capacidad para 2 miembros
                const newVoiceChannel = await guild.channels.create({
                    name: `Trio de ${member.displayName}`,
                ...
                for (const channelId in voiceChannels) {
                    ...
                    if (channel.type === 'GUILD_VOICE' && channel.name.toLowerCase().startsWith('trio de')) {
                        lastTrioChannel = channel;
                    }
                }

                if (lastTrioChannel) {
                    // Si se encontró un canal "Duo de", colocar el nuevo canal después de él
                    await newVoiceChannel.setPosition(lastTrioChannel.position + 1);
                } else {
                    // Si no se encontró ningún canal "Duo de", colocar el nuevo canal después del canal "crear duo"
                    await newVoiceChannel.setPosition(crearTrioChannel.position + 1);
                }
